SELECT j.janr_name, count(ge.executor_id) FROM janr j  
   JOIN janr_executor ge ON j.id = ge.janr_id
   GROUP BY j.janr_name
...
SELECT a.album_name, AVG(t.dlitelnost) FROM album a
	JOIN track t ON a.id = t.album_id
...
SELECT c.collection_name FROM collection_of_songs c
   ...
   WHERE e.executor_name LIKE '%%Дора%%'
  
SELECT a.album_name FROM album a
	JOIN executor_album ea ON a.id = ea.album_id
	JOIN executor e ON e.id = ea.executor_id
	JOIN janr_executor ge ON ge.executor_id = e.id
	JOIN janr g ON g.id = ge.janr_id
	GROUP BY e.executor_name, a.album_name
    HAVING count(ge.janr_id) > 1
...
SELECT e.executor_name FROM executor e
	...
	WHERE dlitelnost = (SELECT MIN(dlitelnost) FROM track)
